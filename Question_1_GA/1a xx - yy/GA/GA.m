function [avg_fitness, max_fitness, x, y] = GA(No_of_Generation, Pop_Size)
...
for k = 1:No_of_Generation
    ...
    for i = 1:Pop_Size
        ...
        x2 = scaling(dec(pop0(c1,17:end)));
        
        y1 = scaling(dec(pop0(c2,1:16)));
        y2 = scaling(dec(pop0(c2,17:end)));
        
        if obj(x1,x2) <= obj(y1,y2)
            mat(i,:) = pop0(c1,:);
        else
            mat(i,:) = pop0(c2,:);
        end
